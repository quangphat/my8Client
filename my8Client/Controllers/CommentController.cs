...
namespace my8Client.Controllers
{
    ...
    public class CommentController : BaseController
    {
        ...
        [HttpGet]
        [Route("{Id}/{postType}/{skip}")]
        public async Task<IActionResult> GetCommentByPostId(string Id,int postType,int skip)
        {
            var result = await _httpClient.SendRequestAsync<ResponseJsonModel<List<Comment>>>(Request, _clientConfig, $"/comments/{Id}/{postType}/{skip}", HttpMethod.Get);
        ...
        public async Task<IActionResult> CreateComment([FromBody] Comment model)
        {
            ...
            var result = await _httpClient.SendRequestAsync<ResponseJsonModel<Notification>>(Request, _clientConfig, "/comments", HttpMethod.Post, model);
            return ToResponse(result);
        }
    }
}